...
- [training/evaluate_saved_model.sh](/home/murasame/nas/pythonproject/ChatTime/training/evaluate_saved_model.sh)：对已保存模型或日志目录做评测，支持多卡
- [training/evaluate_dam_model_debug.sh](/home/murasame/nas/pythonproject/ChatTime/training/evaluate_dam_model_debug.sh)：小样本调试评测入口
...
- `training/finetune_dam_1h_small.sh` 每次都会重建一个小数据集，默认输出到 `dataset/dam_1h_dx_sft_small`
- `training/finetune_dam_1h.sh` 只有在数据集不存在，或者 `context_feature_scope` 与现有 `metadata.json` 不一致时，才会自动重建数据集
...
便捷封装脚本：
[training/evaluate_saved_model.sh](/home/murasame/nas/pythonproject/ChatTime/training/evaluate_saved_model.sh)

调试脚本：
[training/evaluate_dam_model_debug.sh](/home/murasame/nas/pythonproject/ChatTime/training/evaluate_dam_model_debug.sh)
...
`training/evaluate_saved_model.sh` 适合在训练完成后单独评测，不需要手动拼完整参数。它支持两种输入：

- 传入一个可直接加载的模型目录
- 传入一个日志目录，脚本会自动选择最新的 `checkpoint-*`

默认评测 `test`，也可以显式传 `train / validation / test`：

```bash
bash training/evaluate_saved_model.sh outputs/dam_1h_dx_multigpu test
```

或者：

```bash
bash training/evaluate_saved_model.sh logs/dam_1h_dx_multigpu validation
```

输出默认保存在输入目录下的 `eval_<split>.json`，同时还会保存：

- `eval_<split>.json`
- `eval_<split>_predictions.json`

### 多卡评测

`training/evaluate_saved_model.sh` 支持多卡。设置 `GPU_IDS` 后，脚本会自动切换到 `torchrun`，并由各进程分片评测、最后汇总指标。
...
```bash
GPU_IDS=0,1 NPROC_PER_NODE=2 bash training/evaluate_saved_model.sh logs/dam_1h_dx_multigpu test
```

补充说明：

- 这里的 `BATCH_SIZE` 是每个进程的 batch size，不是全局总 batch size
- `training/finetune_dam_1h_multigpu.sh` 训练结束后的自动评测仍是单进程；如果想对保存后的模型做多卡评测，请单独调用 `training/evaluate_saved_model.sh`

### 小样本调试评测

如果只是想快速检查输出格式、定位生成异常或观察日志，可以用：
...
- 使用 `outputs/dam_1h_dx_small` 和 `dataset/dam_1h_dx_sft_small`
- 只评测 `10` 条样本
- `log_interval=1`，便于逐步观察
